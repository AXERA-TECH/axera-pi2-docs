...
下载完成后，解压出系统烧录的axp文件，如![](media/board/axp.png)

下载完成后，SD卡烧写文件，如![](media/board/烧写4.png)
...
![](media/board/axdl1.png)

步骤2：工具栏单击axp加载按钮![](media/board/axdl2.png)，选择.axp镜像包文件![](media/board/axdl3.png)

axp加载中![](media/board/axdl4.png)

步骤3：工具栏单击“设置”![](media/board/axdl5.png)按键，在“Setting”页面AXDL将axp镜像包镜像文件释放到本地Temp目录并自动进行配置，加载完成后用户可以鼠标双击或者**右键单击**镜像文件，激活文件选择框重新选择下载镜像文件![](media/board/axdl6.png)

步骤4：爱芯派2（AXera—Pi 2）目前仅支持USB下载，切换到“Option”页面，配置如图所示：

![](media/board/axdl7.png)
...
步骤5：单击“OK”确认设置，在工具栏单击“开始”![](media/board/axdl8.png)按钮启动下载，此时给开发板上电，下载工具将自动与开发板握手交互，开始下载。
...
![](media\board\SD卡烧录.png)

2，点击`选择目标磁盘`按钮，选择对应的Micro SD存储卡作为目标存储设备

![](media/board/SD卡烧录1.png)

![](media/board/SD卡烧录2.png)

3，点击`现在烧录！`按钮开始烧录，待工具提示`烧录完成！`时，表示镜像烧录完成，可以关闭balenaEtcher并取出存储卡

![](media/board/SD卡烧录3.png)

![](media/board/SD卡烧录4.png)

![](media/board/SD卡烧录5.png)
...
![](media/board/SD卡烧录6.png)

![](media/board/SD卡烧录7.png)
...
![](media/board/type-C版本debug图片.jpg)
...
- 当串口USB转接板首次插入电脑时，需要安装串口驱动；驱动安装完成后，设备管理器可正常识别串口端口，如下图所示：
  ...
  ![](E:/AX630C/git/axera-pi2-docs/source/media/board/远程登录串口.png)

- 打开`MobaXterm`工具，点击`Session`，然后选择`Serial`

- 配置端口号，例如`COM4`，实际使用的串口号以PC识别到的串口号为准。如下图所示：

  ![](media/board/远程登录串口设置.png)
